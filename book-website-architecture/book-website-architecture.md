...
#### 第7章 随需应变：网站的可扩展架构
43.网站的扩展性和伸缩性：
扩展性：对现有系统影响最小的情况下，系统功能可持续扩展或提升的能力。表现在系统基础设施稳定不需要经常变更，应用之间较少依赖和耦合，对需求变更可以敏捷响应。是系统架构设计层面的开闭原则。
伸缩性：系统能够通过增加或减少自身资源规模的方式增强或减少计算处理事务的能力。在网站架构中，通常指利用集群的方式增加服务器数量、提高系统整体事务吞吐能力。

44.低耦合的系统更容易扩展，低耦合的模块更容易复用，一个低耦合的系统也会让开发和维护变得更加简单。

45.架构师的价值在于具有将一个大系统切分成多个低耦合的子模块的能力，这些子模块包含横向的业务模块，也包含纵向的基础技术模块。

46.分层和分割可实现架构伸缩，也是模块化设计的重要手段。
利用分层和分割将软件分割成若干个低耦合的独立的组件模块，这些模块以消息传递及依赖调用的方式聚合成一个完整的系统。这些模块通过分布式部署的方式，独立的模块部署在独立的服务器（集群）上，从物理上分离模块之间的耦合关系，进一步降低耦合性提供复用性。

47.模块分布式部署后聚合的方式有分布式消息队列和分布式服务。
分布式消息队列：通过消息对象分解系统耦合性，不同子系统处理同一个消息
分布式服务：通过接口分解系统耦合性，不同子系统通过相同的接口描述进行服务调用。

48.事件驱动架构：
Event Driver Architecture，通过在低耦合的模块间传输事件消息，以保持模块的松散耦合，并借助事件消息的通信完成模块间合作。
典型的EDA架构就是操作系统中常见的生产者消费者模式。

49.分布式消息队列：
伸缩性：消息队列服务器上的数据是即时处理的，类似于无状态的服务器，伸缩性设计比较简单，增加或减少服务器即可。
...
51.应用拆分的两种方式：
纵向拆分：将一个大应用拆分成多个小应用，如果新增业务比较独立，则直接设计部署为一个独立的web应用系统。
较简单，通过梳理业务，将较少相关的业务剥离，使其称为独立的web应用。
横向拆分：将复用的业务拆分出来，独立部署为分布式服务，新增业务只需要调用这些分布式服务，不需要依赖具体的模块代码，即可快速搭建一个应用系统。模块内业务逻辑变化时，只要接口保持一致就不会影响业务程序和其他模块。
