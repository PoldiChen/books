...
5.Redis两种不同形式的持久化：<br>
(1) 时间点转储<br>
在指定时间段内有指定数量的写操作时执行；也可以通过调用两条转储到硬盘命令中的任何一条来执行<br>
(2) 将所有修改了数据的命令都写入一个只追加的文件里面<br>

6.Redis主从复制：<br>
扩展读性能，并提供故障转移支持。<br>
执行复制的从服务器会连接上主服务器，接收主服务器发送的整个数据库的初始副本，之后主服务器执行的写命令，都会发送到从服务器去执行，从而实时的更新从服务器的数据集。<br>
从服务器可用户读操作，避免对主服务器的集中式访问。
...
#### 第2章 使用Redis构建Web应用
9.一个web服务器对请求响应的典型步骤：<br>
(1) 服务器对客户端发来的请求（request）进行解析<br>
(2) 请求被转发给一个预定义的处理器（handler）<br>
(3) 处理器可能从数据库中读取数据<br>
(4) 处理器根据取出的数据对模板（template）进行渲染（render）<br>
(5) 处理器向客户端返回渲染后的内容作为对请求的响应（response）<br>

10.web请求被认为是无状态的，服务器不会记录与过往请求相关的信息。

11.使用更快的Redis查询代替传统的数据库查询，使用Redis完成一些关系型数据库无法高效完成的任务，提高系统响应实时请求的性能。

12.对于登录用的cookie，有两种常用的方式将登录信息存储在cookie中：签名（signed）cookie和令牌（token）cookie。

cookie类型 | 描述 | 优点 | 缺点
-|-|-|-
签名cookie | 通常会存储用户名，可能还有用户ID、用户最后一次登录时间以及其他有用信息 | 验证cookie需要的所有信息都存储在cookie中，可以包含额外的信息，对这些信息进行签名也很容易 | 正确的处理签名很难，忘记签名或验证签名会造成安全漏洞
令牌cookie | 在cookie中存储一串随机字节作为令牌，服务器根据令牌在数据库查找令牌的拥有者 | 添加信息很容易，cookie的体积很小，较慢的客户端也可以更快的发送请求 | 需要在服务器中存储更多信息，如果是关系型数据库，读写cookie代价较高

#### 第二部分 核心概念
#### 第3章 Redis命令
13.字符串命令：字符串可以存储3种类型的数值：
- 字符串
- 整数
- 浮点数
...
INCR | 将键存储的值加1
DECR | 将键存储的值减1
INCRBY | 将键存储的值加整数amount
DECRBY | 将键存储的值减整数amount
INCRBYFLOAT | 将键存储的值加浮点数amount

14.处理子串和二进制位的命令：

命令 | 行为
-|-
APPEND | 在key对应的value末尾追加
GETRANGE | 根据偏移量获取子串
SETRANGE | 根据偏移量设置子串
GETBIT | 将字符串看做二进制位串，并返回偏移量的二进制位的值
SETBIT | 设置二进制位的值
BITCOUNT | 统计值为1的二进制位数
BITOP | 二进制位运算

15.阻塞式列表弹出命令，常用于消息传递和任务队列

命令 | 行为
-|-
BLPOP | 从非空列表中弹出左端元素，在指定时间内阻塞等待可弹出的元素
BRPOP | 从非空列表中弹出右端元素，在指定时间内阻塞等待可弹出的元素
RPOPLPUSH | 从源列表弹出最右端元素，推入目标列表的最左端
BRPOPLPUSH | 在指定时间内等待可弹出元素
